    ...
    public void retrieveData(DistributedSiddhiQuery distributedSiddhiQuery,
        ...
        try {
            for (SiddhiAppHolder appHolder : appsToDeploy) {
                int executionGroup = Integer.valueOf(appHolder.getAppName().substring(appHolder.getAppName().length()-3,
                        appHolder.getAppName().length()-2));
                int parallelInstance = Integer.valueOf(appHolder.getAppName().substring(appHolder.getAppName().length()-1));
                logger.info("Metric details of " + appHolder.getAppName() + "\n");
                logger.info("---------------------------------------------------");
                String query = "SELECT entire_lat,m14 FROM metricstable where exec=" +
                ...
                if ( resultSet.isBeforeFirst()){     //Check the corresponding partial siddhi app is having the metrics
                    while(resultSet.next()) {
                        double latency = resultSet.getLong("entire_lat");
                        logger.info("latency : " + latency);
                        latency_map.put(appHolder.getAppName() , latency);
                        double processCPU = resultSet.getDouble("m14");
            ...
            try {
                for (SiddhiAppHolder appHolder : appsToDeploy) {
                    ...
                    if ( resultSet.isBeforeFirst()){
                        while(resultSet.next()) {
                            double latencyDB = resultSet.getLong("entire_lat");
